...
// wrapping steady in both directions.
export const useTypewriter = (target: string, speed = 8) => {
  const [state, setState] = useState({ text: "", line: target });
    ...
    const step = (current: typeof state) =>
      current.text === target
        ...
        : !target.startsWith(current.text)
          ? { text: current.text.slice(0, -1), line: current.line }
          : { text: target.slice(0, current.text.length + 1), line: target };
